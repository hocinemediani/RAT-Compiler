...
let string_of_info info =
  ...
  | InfoFun (n, t, tp) -> 
    "Fonction " ^ n ^ " : " ^ 
      (List.fold_right 
        (fun (elt, is_ref) tq -> 
           let type_str = (if is_ref then "ref " else "") ^ (string_of_type elt) in
...
let modifier_type_variable t i =
    match !i with
    | InfoVar (n,_,dep,base,ref) -> i:= InfoVar (n,t,dep,base,ref)
...
(* Modifie l'emplacement (dépl, registre) si c'est une InfoVar, ne fait rien sinon *)
 let modifier_adresse_variable d b i =
     match !i with
     | InfoVar (n,t,_,_,ref) -> i:= InfoVar (n,t,d,b,ref)
